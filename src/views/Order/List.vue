<template>
  ...
  >
    ...
    :title="$t('order.name', { code: detail?.data?.data?.data?.code })"
    :destroyOnClose="true"
    :closable="true"
    @cancel="hideModal"
    @ok="hideModal"
...
import { useGetList, useDetail } from "@/hooks/order";
import {ref} from "vue"
const entity = "order";
...
interface BodyStyleModal {
  height: string;
  padding:string;
}
const bodyStyleModal = ref<BodyStyleModal>({
  height: "72vh",
  padding:"0"
});
